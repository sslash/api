...
open (DELFILE, '>>deleteDB.sql');
open (MYFILE, '>>populateTestDB.sql');
open (SHREDS, '>>populateShredsTest.sql');
open (BATTLES, '>>populateBattlesTest.sql');
open (FANS, '>>populateFansTest.sql');
open(BATTLEREQ, '>>populateBattleRequestsTest.sql');
open (SHREDDERS, '>>populateShreddersTest.sql');


my $dropSQL = "DELETE FROM Fan;\n".
...
for (my $i = 0; $i < 10; $i++) {
    ...
    my $guitar = "INSERT INTO GuitarForShredder VALUES ('$g[$rand1]', $shredder);\n";
    my $equiptment = "INSERT INTO EquiptmentForShredder VALUES ('$eq[$rand2]', $shredder);\n";
...
# Create 1000 shreds SELECT Id FROM Shredder ORDER BY RANDOM() LIMIT 1;
for (my $i = 0; $i < 1000; $i++) {
...
# create battles
my $size = 10; # size of shredders! Add updated value here!
for ( my $i = 0; $i < 1000; $i++) {
    ...
    print BATTLES $battleSQL.$shredSQL.$shredForBattleSQL.$rating;
}

# Create fanees

# Special function that needs the shredders
my @shredders = ( "4087",
"4088",
"4089",
"4090",
...
"4095",
"4096");
my $size = scalar @shredders;
print "size: $size\n";
for ( my $i = 0; $i <$size; $i++) {
    
    # Max 10 fanees
    
    my $faneeNum = int(rand($size));
    
    my %faneesHash = ();
    for ( my $y = 0; $y < $faneeNum; $y++) {
        my $fan = int(rand($size));
        print "Adding: $i : $fan";
        
        # only add to hash fanee = i
        if ( $fan != $i ) {
            print "Adding: $i : $shredders[$fan]";
            $faneesHash{$shredders[$fan]} = $shredders[$fan];
        }
    }
    
    #create sql strings
    while ((my $key, my $value) = each(%faneesHash)){
        my $fanSQL = "INSERT INTO Fan VALUES(".
        "$shredders[$i], $key, DEFAULT);\n";
        print FANS $fanSQL;
    }    
}
